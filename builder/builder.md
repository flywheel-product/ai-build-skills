...
- **Every app is versioned, and the version is visible in the app.** Show the semver version small and unobtrusive in the footer (or the app's natural equivalent — an About screen, a `--version` flag for a CLI). `package.json` (or the platform's manifest) is the single source of truth; the UI reads it at build time — never a hardcoded copy that can drift. If an app has no visible version yet, add one with the next ship.
- **Bump the version on every ship to the default branch.** Decide the level yourself, don't ask: **patch** for fixes/copy/chores, **minor** for new features or notable UX changes, **major** for breaking changes or a reworked experience. Keep every `package.json` in a multi-package repo in sync.
- **When shipping a feature batch, produce release notes** — a short, plain-language summary of what changed and why it helps, ready to drop into Slack/email. The prompter ships to a team and will ask for these; offer them proactively.
